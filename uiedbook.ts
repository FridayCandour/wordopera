...
export const lit = function (
  type: unknown[] | Partial<ObjectConstructor>,
  label: string | number,
) {
    ...
    if (Array.isArray(type) && Array.isArray(value)) {
      ...
      for (let i = 0; i < type.length; i++) {
        if (typeof value[i] === type[i] || value[i] === type[i]) {
          ...
          console.warn(
            `WARNING:-: type ${value} is not assignable to type ${type} at ${label}`,
          );
        }
      }
    } else {
      ...
      for (const k in type) {
        if (typeof value[k] === type[k] || value[k] === type[k]) {
          ...
          console.warn(
            `WARNING:-: the object types for type and value are not assignable at ${label}`,
          );
          break;
        }
      }
...
export const t = function (...args: string[]) {
  ...
  return function (value: unknown) {
    if (!Array.isArray(type)) {
      ...
      if (value === type || typeof value === type) {
        ...
        console.warn(
          `WARNING:-: type ${typeof value} is not assignable to type ${type} at ${label}`,
        );
      }
    } else {
      // for union types
...
type Uied = {
  each(
    fn: { call: (arg0: NodeListOf<HTMLElement>, ind: number) => void },
  ): void;
  ...
  attr(
    attribute_object: Partial<HTMLElement>,
...
export const u = (el: string | BaseE, ifAll_OrNum?: boolean | number): Uied => {
  ...
  return {
    each(
      fn: {
        call: (arg0: NodeListOf<HTMLElement> | HTMLElement, id: number) => void;
      },
    ) {
      if (all) {
        e.forEach((el, ind) => {
...
export const build = (...layouts: lay | lay[]) => {
  let i = 1;
...
export const isEmptyObject = (
  obj: unknown,
): obj is Record<string | number | symbol, never> =>
...
export const buildCanvas = function (
  ...
  const canv = document.createElement("canvas"),
    ...
    backingRatio = backingStores.reduce(function (prev, curr) {
      ...
      return context?.hasOwnProperty(curr)
        ? (context[curr as keyof CanvasRenderingContext2D] as unknown as string)
        : 1;
    }),
...
export const game = (function () {
 ...
 sequenced on.*/
  ...
  const start = (canvas: HTMLCanvasElement, fps = 0) => {
    if (!canvas) {
      throw new Error(
        "uiedbook: cannot start game without a canvas. EXP game.start(canvas)",
      );
    }
...
but these are stationary
  */
  if (!name || !img || !map) {
    throw new Error(
      "cannot create entity shader without a map or image objects",
    );
  }
  ...
  this.config = function (
    ...
    if (!top || !left || !bottom || !right) {
      throw new Error(
        `uiedbook: entityShader.config(top, left, bottom, right) on ${this.name} is invalid`,
      );
    }
    this.left = left;
    this.top = top;
        ...
        for (let j = 0; j < entityArray.length; j++) {
          if (entityArray[j].name === ent.name) {
            ...
            for (let i = 0; i < map.top.length; i++) {
              if (
                !(map.top[i] + reduce >
                  entityArray[j].top + entityArray[j].height)
              ) {
            ...
            for (let i = 0; i < map.top.length; i++) {
              if (
                !(map.left[i] - reduce >
                  entityArray[j].left + entityArray[j].width)
              ) {
                // set the bomb here dev
                detected(j);
                break;
...
export const renderer = (function () {
  ...
  function copyCanvasTo(
    ...
    if (!c) {
      throw new Error(
        "uiedbook: the main game canvas cannot be copied to a null element",
      );
    }
  ...
  function animate(dt: number) {
    ...
    if (calcFPS(dt)) {
      try {
        ...
        throw new Error(
          `the canvas cannot be animated due to some errors | ${error}`,
        );
      }
    }
  }
